...
$mailcontent1 	= isset($_POST['mailcontent']) 	? $_POST['mailcontent'] : "" ;
...
if ($req == "login") {
	if (!isset($_SESSION['user'])) {
		if ( $_SERVER['REQUEST_METHOD'] == 'POST' ) {
			$user = $_POST['u'];
			$pass = $_POST['p'];
			if ($user == 'hr-manager' && $pass == 'HRM-Manager@21') {
				$_SESSION["user"] = "spe";
...
/**************************************************** */
if ($req == "fetch") {
	$stmt = $con->prepare("SELECT * FROM recruit_applicants  $fetch $fetch2 ");
	$stmt->execute();
	//$rows = $stmt->fetchAll();
	...
	$json_data = array();
      foreach ($rows as $row) {
        ...
        $json_arrey['Who_invited_you'] = $row['q4'];
...
if ($req == "add") {
	...
	$stmt = $con->prepare('INSERT INTO  recruit_applicants(name,university,faculty,department,level,mobile,gmail,uniemail,face, q0 ,q1, q2, q3, q4, q5, q6, q7, q8, status , date ,emailed )
...
/**************************************************** */
if ($req == "delete") {
	$stmt = $con->prepare('DELETE FROM recruit_applicants WHERE id = ?');
...
if ($req == "edit") {
	...
	$stmt = $con->prepare('UPDATE recruit_applicants SET name=?,university=?,faculty=?,department=?,level=?,mobile=?,gmail=?,	uniemail=?,face=?, q0 =?,q1=?,q2=?,q3=?,q4=?,q5=?,q6=?,q7=?,q8=?,comment=?  WHERE id = ?');
...
if ($req == "accept") {
	...
	$stmt = $con->prepare('UPDATE recruit_applicants SET status=? WHERE id = ?');
...
if ($req == "mailcheck") {
	...
	$stmt = $con->prepare('UPDATE recruit_applicants SET emailed=? WHERE id = ?');
...
if ($req == "mail") {
	...
	try {
			...
			$mail->Password = 'hrm.20202021';                      		// SMTP password
