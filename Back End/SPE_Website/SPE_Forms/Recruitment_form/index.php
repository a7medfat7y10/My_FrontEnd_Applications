<?php
	$do = isset($_GET['do']) ? $_GET['do'] :"off"; // (  off , form ) to off form 
	include "connect.php" ;
	$ok = "no";
	if ($_SERVER["REQUEST_METHOD"] == 'POST') {
		...
		$stmt->execute(array( "zname"=>$name, "zuniversity"=> $university, "zfaculty"   =>$faculty, "zdepartment"=>$department, "zlevel" =>$level, "zmobile" =>$mobile, "zgmail" =>$gmail, "zuniemail" =>$uniemail, "zface" =>$face, "zq0"=>$q0 ,"zq1" => $q1, "zq2"=>$q2, "zq3"=>$q3, "zq4"=>$q4, "zq5"=>$q5, "zq6"=>$q6, "zq7"=>$q7, "zq8"=>$q8, "zstatus"=>"", "zdate"=> Date('Y-m-d H:i:s'), "zemailed"=>""));
		$count = $stmt->rowCount();
		...
		if ($count > 0) {
			...
			//exit();
			$ok = "ok";
			header("refresh:1;url=index.php?do=done");
		}
	}
?>       
<!DOCTYPE html>
	...
	<body>
										...
										<form  id="forms" method='POST' action="<?php ?>">
											...
											<div class="tab">
												...
												<div class="input-field">
													...
													<label for="q4">Who invited you to join SPE Suez?(Please write his/her name)</label>
												</div> 
											</div>
						...
						<div class="row center-all" style="margin-bottom: 0;">
							<div class="col m12 s12 center">
								<div class="center white-text">
									<img src="img/logo2.png" width="35%" alt="">
									<img src="img/recruit.png" width="80%" alt="">
									<h4>Sorry, The Form is currently not accepting responses </h4>
								</div>
							</div>
						</div>
					</div>
		...
		<script>
			...
			function validateForm() {
				...
				y = x[currentTab].getElementsByClassName('validate');                          // .getElementsByTagName("input");
